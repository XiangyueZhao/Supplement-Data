...
for k=1:length(powerall)
    ...
    save_path = ['G:\BIDS\ERP\ANT\POWER\' powername];% change your path
%     t=[];
%     for subi = 1:26
%         behav_data = tdfread('G:\bids_ALL_result\BIDS_26_ByBiosemi\participants.tsv');% change your path
%         sub = behav_data.participant_id;
%         input_path = 'G:\bids_ALL_result\derivatives\26\rerefer\';% change your path
...
%     end
    t=[];
    for subi = 1:29
        behav_data = tdfread('G:\BIDS_bigdata\BIDS_29_ByANT\participants.tsv');% change your path
        sub = behav_data.participant_id;
        input_path = 'G:\BIDS_bigdata\BIDS_29_ByANT\derivatives\rerefer\';% change your path
        file_name = [sub(subi, :) '_29ByANT.set'];
        rating_screen (file_name,input_path,save_path,power,t);
    end
%     t=[];
%     for subi = 1:30
